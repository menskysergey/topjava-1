...
- 11: В `assertMatch` для модели нельзя использовать `assertThat().isEqualTo`: сравнение будет происходить через `equals`, который сравнивает объекты только по `id`. Мы не можем переопределять `equals` для объектов модели, тк будем использовать JPA (на следующем уроке будет ссылка)
